...
export function SupplyTimeline({ items, stages }: SupplyTimelineProps) {
  ...
  if (items.length === 0) {
    return (
      <div className="text-center py-12 text-[13px] text-[#9B9B9B]">
  ...
  return (
    <div className="bg-white border border-[#E8E6E1] rounded-xl overflow-hidden">
      {/* Month header */}
      <div className="relative h-8 border-b border-[#F0EEE9] bg-[#FAFAF8]">
        {months.map((m, i) => (
          <div
            key={i}
            className="absolute top-0 h-full flex items-center text-[10px] text-[#9B9B9B] font-medium"
            style={{ left: `${m.leftPct}%` }}
          >
            <div className="w-px h-full bg-[#F0EEE9]" />
      ...
      <div className="relative">
        ...
        {rows.map((item) => {
          ...
          return (
            <div
              key={item.id}
              className="relative h-[44px] border-b border-[#F0EEE9] last:border-none flex items-center"
            >
              {/* Label */}
              <div className="absolute left-2 z-10 text-[11px] font-medium text-[#6B6B6B] truncate max-w-[140px]">
                {item.name}
              </div>
              {/* Bar */}
              <div
      ...
      {/* Legend */}
      <div className="flex items-center gap-4 px-4 py-2.5 bg-[#FAFAF8] border-t border-[#F0EEE9]">
        {(["critical", "high", "medium", "low"] as const).map((level) => (
          <div key={level} className="flex items-center gap-1.5">
            ...
            />
            <span className="text-[10px] text-[#9B9B9B]">{RISK_CONFIG[level].label}</span>
          </div>
        ))}
        <div className="flex items-center gap-1.5 ml-2">
          <div className="w-3 h-0.5 bg-[#E85D3A] rounded" />
          <span className="text-[10px] text-[#9B9B9B]">Сегодня</span>
        </div>
      </div>
    </div>
  );
