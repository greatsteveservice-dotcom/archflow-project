...
async function cleanTranscript(transcript: string): Promise<string> {
  const res = await fetch('https://api.anthropic.com/v1/messages', {
    ...
    body: JSON.stringify({
      ...
      max_tokens: 600,
      system: `Ты редактор. Твоя задача — превратить сырую расшифровку голосового сообщения в чистый, лаконичный текст.

Правила:
1. Выдели СУТЬ сообщения — что человек хочет сказать/предложить/спросить
2. Удали ВСЁ лишнее: слова-паразиты, повторы, отступления, самокоррекции, размышления вслух
3. Переформулируй своими словами — НЕ пересказывай фразу за фразой
4. Сохрани все факты, имена, числа, даты
5. Результат: 2-4 предложения максимум
6. Без маркдауна, без кавычек, без вступлений типа "Суть сообщения:"`,
      messages: [
        {
          role: 'user',
        ...
        {
          role: 'assistant',
          content: 'Нужно перезаказать керамогранит 60x60 — пришло 8 коробок вместо 12, и цвет отличается от образца.',
        },
